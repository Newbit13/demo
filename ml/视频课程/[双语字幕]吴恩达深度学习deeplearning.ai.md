...
列举每个约定符号的维数
# p40 为什么使用深层表示
作者用例子直观解释了为什么深层的表现更好：
1. 第一层隐藏层用来检测不同类型线条->第二层检测由这些线条组成的鼻子，眼睛等->第三层检测由这些组成的人脸->输出结果。
2. 语音识别，从音位到单词到句子，从简单到复杂，从浅层到深层，也是同样的道理需要深层神经网络
可以看到深层神经网络可以表示的意思更加的具体，准确

从电路理论来讲，如果只能用单个隐藏层的话，需要的神经元对比可以使用多层的，需要更多

作者拿到个新问题时会选择先从logistc回归开始，在尝试着加一两层隐藏层。他表示深度学习这个“高大上”的词只不过是神经网络的一个包装
# p41 Building blocks of deep neural networks
讲解如果写代码实现神经网络，需要写什么函数，输入什么，输出什么（跟之前的双层神经网络作业很想）
p43 00:19 有前向传播+反向传播的核心公式
# p42 Parameters vs Hyperparameters
Hyperparameters:学习率α、梯度下降迭代次数、层数l、神经元数、激活函数（其他以后会讲的：momentum、mini batch、regularzation正则化参数等）
超参数得靠自己试才知道什么值最佳（多试能试出经验。。。额(⊙﹏⊙)）
# p43 和大脑有关系吗
至今即使是神经专家也无法解释一个神经元能做什么，会做什么（按我理解就是不知道他们是什么激活函数）
# p44
# p45
# p46
# p47 训练集、开发集（验证集）、测试集
以前按照6:2:2划分训练、验证、测试集
在大数据时代（比如数据是百万级），那么用98%,1%,1%来划分也合理（数据够多时，训练集比例还可以再高，因为后面两个集主要是起调整超参，验证算法，还有评估效果的作用）
# p48 偏差和方差（bias and variance）
通过比较训练集和测试集的误差，来判断是高偏差还是高方差：
...
这个有个基本的错误率作为前提，就是我们假设人眼错误率是0%。（并且假设数据来源分布相同）如果人眼判断的错误率是15%的话，那么训练集误差达到15%时我们也会认为正常

# p49机器学习的基本“诀窍”
判断是高方差还是高偏差，然后采取对应措施，不断尝试，直到找到低方差+低偏差的框架
在现在的大数据时代，就主要有更大的网络+更多的数据就可以了（前提是有合适的正则化）
# p50 正则化
lambda是python的关键字，所以所以一般用lambd代替

L2 regularization：$原来的代价函数 + \frac{λ}{2m}||w||^2$

其中：$||w||^2 = \sum_{j=1}^{nx}w_{j}^2$

在神经网络中的L2 regularization: $原来的代价函数 + \frac{λ}{2m}\sum_{l=1}^{L}||w^{[l]}||_F^2$

其中：$||w^{[l]}||_F^2 = \sum_{i=1}^{n^{[l-1]}}\sum_{j=1}^{n^{[l]}}(w_{ij}^{[l]})^2$

$||w^{[l]}||_F^2$被称为Frobenius norm of a matrix

原来的$w^{[l]}$更新会增加一项：$-\frac{αλ}{m}w^{[l]}$

# p51 为什么正则化可以减少过拟合
1. λ越大，得到的w越接近0，相当于减少了部分特征数，使得过拟合的情况往欠拟合发展
2. 如果λ变大，那么w变小，z变小，在tanh激活函数中会导致 即使神经网络层数很深，也会由于这样导致整个假设函数趋于线性，所以会变得不那么过拟合

# p52 Dropout 正则化
随机删掉神经网络中的一些节点，使得网络更加简单
