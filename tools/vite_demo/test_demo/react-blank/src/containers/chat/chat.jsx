...
class Chat extends Component{
    ...
    render(){
        ...
        const thatChatList = this.props.ChatList.chatList.filter((chat)=>chat.chat_id === chat_id)
        return (
            <div>
                ...
                <List>
                    {thatChatList.map((chat,index)=>{
                        if(chat.to === userid){
                            return <Item key={index} thumb={headImg}>{chat.content}</Item>
                        }else{
                            return <Item key={index} className='chat-me' extra='我'>{chat.content}</Item>
                        }
                    })}
                </List>
                <div style={{height:50}}></div>
                <div className='bottom-input'>
                    <InputItem placeholder='请输入内容' value={this.state.content} extra={<span onClick={this.send}>发送</span>} onChange={(value)=>{this.setState({content:value})}}/>
                </div>
            </div>
        )
    }
