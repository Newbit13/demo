...
export function user(state = initUser, action: actionType) {
  switch (action.type) {
    case AUTH_SUCCESS:
      return { ...action.data };
    case ERROR_MSG:
...
const initUserList: initUserListType = [];
export function userList(state = initUserList, action: actionType) {
  switch (action.type) {
    case GET_USER_LIST:
      return action.data;
    default:
...
export function ChatList(state = initChatList, action: actionType) {
  switch (action.type) {
    case GET_CHAT:
      return {
        users: state.users,
        ...
        unReadCount:
          action.data.to === action.data.userid
            ? state.unReadCount + 1
            : state.unReadCount,
      };
    case GET_CHAT_LIST:
